...
function checkID(id) {
    let foundID = arrayVKB.indexOf(id);
...
function foundAuthor(jNode) {
    const userID = jNode.getAttribute('data-from-id');
    const foundID = arrayVKB.indexOf(userID);
    if (foundID > -1) {
        console.log("[GosVon Marking for VK] User found in VKB-db: " + userID);
        const pNode = jNode.parentElement;
...
function foundAuthorCM(jNode) {
    const userID = jNode.getAttribute('href').substr(1);
    const foundID = arrayVKB.indexOf(userID);
    if (foundID > -1) {
        console.log("[GosVon Marking for VK] User found in VKB-db: " + userID);
...
function foundAuthorPM(jNode) {
    let userID = jNode.getAttribute('href').substr(1);
    if (userID.indexOf('?') >= 0) {
...
function foundFan(jNode) {
    const userID = jNode.getAttribute('data-id');
    const foundID = arrayVKB.indexOf(userID);
    if (foundID > -1) {
        console.log("[GosVon Marking for VK] User found in VKB-db: " + userID);
...
function foundProfile(jNode) {
    let userID = window.location.pathname.substr(1);
    if (userID.indexOf('?') >= 0) {
        userID = userID.split('?')[0];
    }
    const foundID = checkID(userID);
    if (foundID > -1) {
...
function waitForKeyElements(selectorTxt, actionFunction, bWaitOnce, iframeSelector) {
    ...
    if (targetNodes && targetNodes.length > 0) {
        ...
        targetNodes.forEach(function (node) {
            const examined = node.dataset.vkbExamined; // Set flag to the node for single check
            if (!examined) {
    ...
    // Repeat the checking
    const key = selectorTxt.replace(/[^\w]/g, "_");
